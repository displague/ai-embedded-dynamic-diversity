...
- [x] Add a learned (non-hardcoded) mimicry training track: environmental mimicry, peer mimicry, and threat mimicry from anonymous signals, optimized for transfer fitness rather than explicit imitation labels.
...
- [x] Make CUDA PyTorch install reproducible in `uv` lock workflow (pins `torch` to the explicit PyTorch `cu130` index on Windows/Linux so plain `uv run` keeps the CUDA wheel). (Closes #14)
